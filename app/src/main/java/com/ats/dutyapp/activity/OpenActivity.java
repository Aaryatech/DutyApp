...
public class OpenActivity extends AppCompatActivity implements View.OnClickListener {
    ...
    ArrayList<ChecklistActionDetail> detailList1 =new ArrayList<>();
    Login loginUser;
    Detail model;
    ...
    public static ImageView ivCamera1,ivPhoto1;
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        if (PermissionsUtil.checkAndRequestPermissions(OpenActivity.this)) {
        }

        createFolder();

        try {
            String userStr = CustomSharedPreference.getString(getApplicationContext(), CustomSharedPreference.MAIN_KEY_USER);
            Gson gson = new Gson();
...
//            OpenDetailAdapter adapter = new OpenDetailAdapter(detailList, getApplicationContext());
...
//            recyclerView.setAdapter(adapter);
        ...
        mBroadcastReceiver = new BroadcastReceiver() {
            ...
            public void onReceive(Context context, Intent intent) {
                if (intent.getAction().equals("CHECKLIST_DETAIL")) {
                   // Intent intent1 = getIntent();
                    int pos = intent.getExtras().getInt("pos");
                    new UploadOpenDialog(getApplicationContext(),pos).show();
    ...
    public void onClick(View v) {
        ...
        {
            ArrayList<DetailList> assignedArray = new ArrayList<>();
            final ArrayList<Integer> assignedDetailIdArray = new ArrayList<>();
            final ArrayList<String> assignedDetailNameArray = new ArrayList<>();
            if (assignStaticDetailList != null) {
                if (assignStaticDetailList.size() > 0) {
                    ...
                    for (int i = 0; i < assignStaticDetailList.size(); i++) {
                        if (assignStaticDetailList.get(i).isChecked()) {
                           ...
                           // assignedDetailIdArray.add(assignStaticDetailList.get(i).get());
                           // assignedDetailNameArray.add(assignStaticDetailList.get(i).getEmpFname() + " " + assignStaticEmpList.get(i).getEmpMname() + " " + assignStaticEmpList.get(i).getEmpSname());
                ...
                Log.e("ASSIGN EMP ID", "---------------------------------" + assignedDetailIdArray);
                Log.e("ASSIGN EMP Name", "---------------------------------" + assignedDetailNameArray);

                String empIds = assignedDetailIdArray.toString().trim();
                ...
                String empName = assignedDetailNameArray.toString().trim();
    ...
    public void onClickActivity(Integer position)
    {
           ...
           // Log.e("For", "-------------------------START--------------------");
            if (detailList.get(position).getIsPhoto() == 1) {
                ...
                try{
                    ...
                    new UploadOpenDialog(act,position).show();
                }catch(Exception e){
                    e.printStackTrace();
                }
...
//                        for (int i = 0; i < detailList.size(); i++) {
//                            ChecklistActionDetail checklistActionDetail = new ChecklistActionDetail(0, detailList.get(i).getChecklistHeaderId(), 0, 0, detailList.get(i).getChecklistDesc(), detailList.get(i).getIsPhoto(), 1, photo1, "", sdf.format(System.currentTimeMillis()), 1, 0, 0, "", "");
//                            detailList1.add(checklistActionDetail);
//                        }
//
//                        AlertDialog.Builder builder = new AlertDialog.Builder(OpenActivity.this, R.style.AlertDialogTheme);
...
//                    } else {
//                        Toast.makeText(OpenActivity.this, "Please Attach photo", Toast.LENGTH_SHORT).show();
//                    }
//
//                }
//            });
...
//                    startActivityForResult(intent, 102);
    ...
    public void onActivityResult(int requestCode, int resultCode, Intent data) {
        ...
        if (resultCode == RESULT_OK && requestCode == 102) {
            ...
            try {
                realPath = RealPathUtil.getRealPathFromURI_API19(OpenActivity.this, data.getData());
                Uri uriFromPath = Uri.fromFile(new File(realPath));
                myBitmap1 = getBitmapFromCameraData(data, OpenActivity.this);
